...
const getNormalizedTextVariants = (value: string) => {
  ...
  return Array.from(variants).map((item) => cleanText(item)).filter(Boolean);
};
...
const PRODUCT_COMMERCIAL_INTENT_WORDS = [
  ...
  "اشتري", "شراء", "سعر", "اسعار", "أسعار", "بكام", "بكم", "كام", "كم", "ارخص", "الأرخص", "افضل", "أفضل", "رشح", "اقترح",
  "want", "need", "buy", "find", "search", "show", "price", "prices", "cost", "how much", "cheapest", "best", "recommend", "looking for",
];

const PRODUCT_AVAILABILITY_INTENT_WORDS = [
  "فيه", "يوجد", "هل يوجد", "موجود", "موجودة", "موجوده", "متاح", "متاحة", "متاحه",
  "متوفر", "متوفرة", "متوفره", "عندكم", "عندك", "عندكو", "عندكوا", "عندكوم", "عندنا", "لديكم", "معاكم",
  "available", "availability", "in stock", "do you have", "do u have", "have", "has", "got", "is there", "are there", "any",
];
...
const formatProductPrice = (product: ProductMatch, piRate?: number) => {
  const productSource = cleanText(product.source || "admin").toLowerCase();
  const localPriceEgp = productSource === "service"
    ? toNumber(product.local_price_egp || product.min_amount_egp)
    : toNumber(product.local_price_egp);
  const piPerEgp = toNumber(piRate);

  /*
   * لا نستخدم product.price للمنتجات العادية أو منتجات التجار، لأن هذا الحقل
   * في بعض البيانات القديمة يحتوي سعر الجنيه المصري فيظهر للعميل كأنه Pi.
   * المصدر الوحيد الصحيح لسعر المنتجات العادية والتجار هو:
   * local_price_egp × piRate القادم من الواجهة.
   */
  ...
  if (localPriceEgp > 0 && piPerEgp <= 0) {
    return "السعر قيد التحديث";
  }

  // Fallback ضيق فقط للبيانات التي لا تملك سعرا محليا إطلاقا، وليس للمدير/التجار.
  if (productSource !== "admin" && productSource !== "merchant") {
    const storedPiPrice = toNumber(product.price);
    if (storedPiPrice > 0 && storedPiPrice < 500) {
...
const getProductSortPrice = (product: ProductMatch) => {
  const productSource = cleanText(product.source || "admin").toLowerCase();
  const localPrice = productSource === "service"
    ? toNumber(product.local_price_egp || product.min_amount_egp)
    : toNumber(product.local_price_egp);
  ...
  if (productSource !== "admin" && productSource !== "merchant") {
    const storedPiPrice = toNumber(product.price);
    if (storedPiPrice > 0 && storedPiPrice < 500) return storedPiPrice;
  }
...
const PRODUCT_QUERY_FILLER_WORDS = new Set([
  ...
  // Arabic conversational fillers / intent words that must NEVER become required product terms.
  // Example: "عندكو شاي" should search for "شاي" only, not require the word "عندكو" inside product data.
  "يسال", "يسأل", "استفسار", "استفسر", "بخصوص", "حول", "عايزه", "ممكن", "لو", "سمحت", "لو سمحت",
  "هل", "يا", "من فضلك", "بليز", "اريد", "أريد", "عايز", "عاوز", "عايزة", "عاوزه",
  "عندكو", "عندكوا", "عندكوم", "عندكم", "عندك", "عندكي", "عندنا", "لديكم", "لدينا", "معاكم",
  "يوجد", "موجود", "موجودة", "موجوده", "متوفر", "متوفرة", "متوفره", "متاح", "متاحة", "متاحه",
  "بكام", "بكم", "كام", "كم", "سعر", "اسعار", "أسعار", "تكلفة", "تكلفه",
  "ماركة", "ماركه", "براند", "نوع", "انواع", "أنواع", "موديل", "موديلات", "قسم", "القسم", "فئة", "فئه",
  "رشح", "اقترح", "اعرض", "هات", "ابحث", "دور", "دورلي", "وريني", "ارخص", "الأرخص", "افضل", "أفضل",

  // English conversational fillers / intent words.
  // Example: "Is there any Xiaomi mobile" should search for "Xiaomi mobile" only.
  "is", "are", "there", "any", "do", "does", "did", "you", "u", "have", "has", "got",
  "please", "pls", "can", "could", "would", "send", "give", "get", "find", "search", "show", "list",
  "for", "of", "from", "with", "about", "regarding", "brand", "brands", "model", "models", "type", "types", "kind", "kinds",
  "available", "availability", "stock", "instock", "in", "the", "a", "an", "how", "much", "price", "prices", "cost", "cheap", "cheapest", "best", "recommend",
]);
...
const formatProductsContextRows = (
  ...
  if (!cleanProducts.length) {
    return getProductFallbackBrowseMessage("", isArabic);
  }
  ...
  return cleanProducts
    .map((product, index) => {
      ...
      const tagsText = Array.isArray(product.tags)
        ? product.tags.map((tag) => cleanText(tag)).filter(Boolean).join(", ")
      ...
      return [
        ...
        `   - ${isArabic ? "السعر النهائي" : "Final price"}: ${priceText}`,
...
type StoreSearchTableProduct = ProductMatch & {
  search_rank?: number | null;
};

const buildStoreSearchCandidateQueries = (query: string) => {
  const candidates: string[] = [];
  const seen = new Set<string>();

  const addCandidate = (value: string) => {
    const safeValue = escapeIlikeTerm(stripArabicDefiniteArticle(value));
    const normalized = normalizeIntentText(safeValue);
  ...
  rawValue
    ...
    .filter(Boolean)
    .forEach((line) => {
      const lineTokens = tokenizeProductQuery(line);
      if (lineTokens.length) {
        addCandidate(lineTokens.join(" "));
        lineTokens.forEach(addCandidate);
      } else {
        addCandidate(line);
      }
    });
  ...
  extractProductSearchTerms(query).forEach(addCandidate);
  getMatchedBrandTermsForQuery(normalizeIntentText(query)).forEach(addCandidate);

  return candidates.slice(0, 12);
};

const canUseArrayContainsTagFilter = (value: string) => {
  return Boolean(value) && !/\s/.test(value) && !/[{},]/.test(value);
};

const buildStoreSearchOrFilters = (safeQuery: string, includeTags = true) => {
  const filters = [
  ...
  if (includeTags && canUseArrayContainsTagFilter(safeQuery)) {
    filters.push(`tags.cs.{${safeQuery}}`);
  }

  return filters.join(",");
};

const fetchProductsWithStoreSearchPattern = async (
  supabase: ReturnType<typeof createClient>,
  lastUserQuery: string,
) => {
  const candidateQueries = buildStoreSearchCandidateQueries(lastUserQuery);
  if (!candidateQueries.length) return [] as ProductMatch[];

  const adminSelect = "id, name, name_ar, description, description_ar, image, category, category_id, price, local_price_egp, original_price, in_stock, stock_quantity, shipping_type, tags";
  const merchantSelect = "id, name, name_ar, description, description_ar, category, category_id, local_price_egp, original_price, in_stock, stock_quantity, merchant_id, shipping_type, tags";
  const collectedProducts: ProductMatch[] = [];

  for (const safeQuery of candidateQueries) {
    const adminFilters = buildStoreSearchOrFilters(safeQuery, true);
    const merchantFilters = buildStoreSearchOrFilters(safeQuery, true);
    ...
    const [adminResult, merchantResult] = await Promise.all([
      supabase
        ...
        .eq("in_stock", true)
        .or(adminFilters)
        .limit(MAX_TEXT_PRODUCT_MATCHES),
      supabase
    ...
    if (adminResult.error) {
      console.warn("store-pattern products search skipped:", adminResult.error.message);
    } else if (adminResult.data?.length) {
      collectedProducts.push(
        ...((adminResult.data || []) as StoreSearchTableProduct[]).map((product) => ({
          ...product,
          source: "admin",
          search_rank: Math.max(toNumber(product.search_rank), 2),
        })),
      );
    }

    if (merchantResult.error) {
      console.warn("store-pattern merchant_products search skipped:", merchantResult.error.message);
    } else if (merchantResult.data?.length) {
      collectedProducts.push(
        ...((merchantResult.data || []) as StoreSearchTableProduct[]).map((product) => ({
          ...product,
          source: "merchant",
          search_rank: Math.max(toNumber(product.search_rank), 2),
        })),
  ...
  return uniqueBy(collectedProducts, (product) => `${product.source || "admin"}:${cleanText(product.id)}`)
...
const fetchServiceProductsByText = async (
  ...
  // مهم: لا نكوّن OR ضخم لكل المرادفات مرة واحدة؛ هذا كان يسبب URL طويل جدا ويفشل الطلب في اللوج.
  // نعتمد نفس نمط StoreSearch: استعلامات قصيرة متتابعة بعد تنظيف product_query.
  const candidateQueries = buildStoreSearchCandidateQueries(lastUserQuery).slice(0, 6);
  if (!candidateQueries.length) return [] as ProductMatch[];
  ...
  const collectedServices: ProductMatch[] = [];
  ...
  for (const safeQuery of candidateQueries) {
    ...
    let serviceResult = await supabase
      ...
      .eq("is_active", true)
      .or(serviceFilters)
      .limit(Math.max(MAX_PRODUCT_MATCHES * 3, 24));
    ...
    if (serviceResult.error) {
      ...
      if (retryResult.error) {
        // غالبا الجدول غير موجود أو لا توجد منتجات خدمية؛ لا نغرق اللوج بتفاصيل URL طويلة.
        console.warn("service_products text search skipped:", truncateText(retryResult.error.message, 220));
        break;
      }
    ...
    if (serviceResult.data?.length) {
      collectedServices.push(
        ...((serviceResult.data || []) as ServiceProductRow[])
  ...
  return uniqueBy(collectedServices, (product) => `service:${cleanText(product.id)}`)
    .map((product) => ({
      product,
    ...
    .sort((a, b) => {
      ...
      return getProductSortPrice(a.product) - getProductSortPrice(b.product);
    })
...
const fetchProductsByText = async (
  ...
  console.info("product store-pattern + RPC search", {
    originalQuery: lastUserQuery,
    storeSearchQueries: buildStoreSearchCandidateQueries(lastUserQuery).slice(0, 12),
    requiredGroups,
  ...
  // Search first with the same practical pattern used by the store search component:
  // direct ILIKE on name/name_ar/description/tags, after the AI/tokenizer extracts the clean product query.
  const storeSearchProducts = await fetchProductsWithStoreSearchPattern(supabase, lastUserQuery);
  collectedProducts.push(...storeSearchProducts);

  // Keep the v14 database-level grouped RPC as an additional fallback / strict family layer, not as the only source.
  ...
  if (error) {
    console.warn("search_products_by_groups RPC failed; using store-pattern results:", error.message);
  } else if (data?.length) {
    collectedProducts.push(...((data || []) as ProductSearchRpcRow[]));
  }

  const uniqueProducts = uniqueBy(collectedProducts, (product) => `${product.source || "admin"}:${cleanText(product.id)}`)
    .filter(isProductAvailableForChat);

  return uniqueProducts
    .map((product) => ({
      product,
      score: scoreProductForQuery(product, searchTerms, lastUserQuery) + toNumber((product as ProductSearchRpcRow).search_rank) * 10,
    }))
    .filter((item) => {
      if (!Number.isFinite(item.score)) return false;

      // Store-search results already came from direct DB matching; do not let a generic family query drop valid rows.
      if (isGenericFamilyBrowse && item.score > 0) return true;
...
const enrichProductsWithCategoryNames = async (
  ...
  // لا نستدعي service_categories هنا لأن بعض قواعد البيانات لا تحتوي هذا الجدول،
  // والمنتجات الخدمية لديها اسم قسم افتراضي بالفعل.
  const serviceCategories: CategoryRow[] = [];
...
const getProductFallbackBrowseTarget = (query: string, isArabic: boolean) => {
  const normalized = normalizeIntentText(query);
  const noArticle = stripArabicDefiniteArticle(normalized);
  const hasAny = (terms: string[]) => terms.some((term) => {
    const normalizedTerm = normalizeIntentText(term);
    return normalized.includes(normalizedTerm) || noArticle.includes(normalizedTerm);
  });

  if (hasAny(["موبايل", "موبايلات", "موبيل", "هاتف", "هواتف", "تليفون", "جوال", "mobile", "mobiles", "phone", "phones", "smartphone"])) {
    return {
      label: isArabic ? "قسم الموبايلات" : "the mobile category",
      markdown: isArabic ? "[قسم الموبايلات](/products)" : "[the mobile category](/products)",
    };
  }

  if (hasAny(["شاي", "شاى", "قهوه", "قهوة", "مشروبات", "سوبر ماركت", "supermarket", "tea", "coffee", "beverages"])) {
    return {
      label: isArabic ? "قسم السوبر ماركت والمشروبات" : "the supermarket/beverages category",
      markdown: isArabic ? "[قسم السوبر ماركت والمشروبات](/products)" : "[the supermarket/beverages category](/products)",
    };
  }

  if (hasAny(["شاشه", "شاشة", "شاشات", "تلفزيون", "تليفزيون", "smart tv", "tv", "screen", "television"])) {
    return {
      label: isArabic ? "قسم الشاشات" : "the screens/TV category",
      markdown: isArabic ? "[قسم الشاشات](/products)" : "[the screens/TV category](/products)",
    };
  }

  if (hasAny(["خدمه", "خدمة", "خدمات", "service", "services"])) {
    return {
      label: isArabic ? "قسم المنتجات الخدمية" : "the service products section",
      markdown: isArabic ? "[قسم المنتجات الخدمية](/products?type=service)" : "[the service products section](/products?type=service)",
    };
  }

  return {
    label: isArabic ? "صفحة المنتجات" : "the products page",
    markdown: isArabic ? "[صفحة المنتجات](/products)" : "[the products page](/products)",
  };
};

const getProductFallbackBrowseMessage = (query: string, isArabic: boolean) => {
  const target = getProductFallbackBrowseTarget(query, isArabic);

  return isArabic
    ? `خلّيني أوصل لك لأدق نتيجة ✅ افتح ${target.markdown} وشوف الخيارات المتاحة، أو اكتب اسم المنتج بتفاصيل أوضح مثل: الماركة + الموديل + الحجم/النوع. مثال: "موبايل سامسونج A07" أو "شاي العروسة 250 جم"، وسأبحث لك مباشرة داخل بيانات المول.`
    : `Let me help you get a more accurate result ✅ Open ${target.markdown} to browse available options, or send a more specific product phrase such as brand + model + size/type. Example: "Samsung A07 mobile" or "Al Arosa tea 250g", and I’ll search the mall data directly.`;
};

const isProductBrowseFallbackContext = (value: string) => {
  const normalized = normalizeIntentText(value);
  return normalized.includes(normalizeIntentText("خليني اوصل لك لادق نتيجه")) || normalized.includes(normalizeIntentText("more accurate result"));
};
const fetchProductsContext = async (
  ...
  if (!uniqueProducts.length) {
    return getProductFallbackBrowseMessage(lastUserQuery, isArabic);
  }
...
5. إذا كانت المنتجات المطابقة غير كافية أو ظهر رابط تصفح آمن في السياق، استخدم رابط القسم/المنتجات بدل قول "لا يوجد منتجات" أو "غير متوفر".
6. الأسعار تظهر للعميل بعملة Pi فقط. ممنوع ذكر الجنيه المصري أو EGP أو أي عملة ورقية، وممنوع إعادة حساب السعر أو استخدام أي رقم من الوصف كسعر.
7. حافظ على روابط Markdown كما هي، وإذا ظهر سطر "للمزيد من نفس القسم" فاستخدمه عند الحاجة كزر/رابط إضافي مناسب.
8. إذا لم تجد معلومة أو نتيجة مؤكدة، وجّه العميل لرابط مناسب داخل المول بصياغة إيجابية، ولا تستخدم عبارة "لا يوجد منتجات". شجّعه على كتابة كلمات أدق مثل الماركة + الموديل + الحجم/النوع.
...
21. عند عرض السعر، انسخ السعر كما هو من سطر "السعر النهائي" ولا تضف أرقام ترتيب بجوار Pi.
...
5. If matching products are not enough or a safe browse link appears in context, use the category/products link instead of saying there are no products.
6. Customer-facing prices must be in Pi only. Never mention EGP, Egyptian Pounds, or fiat currencies. Never recalculate prices or use a number from a description as a price.
...
8. If information or confirmed results are missing, guide the customer to a relevant mall link with positive wording instead of saying there are no products.
...
17. If the filtered database results are empty, do not say "no products". Use the safe browse/category link from the context and do not recommend approximate alternatives unless the customer explicitly asks for alternatives.
...
24. When showing prices, copy the value exactly from the "Final price" line and do not append list numbers beside Pi.
...
serve(async (req) => {
  ...
  try {
    ...
    const rawAnalysis: AiIntentAnalysis = suppliedAnalysis
      ? {
          ...defaultAiIntentAnalysis(isArabic),
          ...suppliedAnalysis,
          language: isArabic ? "ar" : "en",
    ...
    // في أسئلة المنتجات نعتمد product_query النظيف من محلل النية فقط.
    // لا ندمج آخر رسالة خام، لأن عبارات التصحيح مثل "اقصد... وليس..." كانت تدخل كشروط بحث وتكسر النتائج.
    const productDataQuery = PRODUCT_SEARCH_INTENTS.has(analysis.intent)
      ? (cleanText(analysis.product_query) || lastUserQuery)
      : lastUserQuery;
    const knowledgeDataQuery = cleanText(analysis.topic) || (PRODUCT_SEARCH_INTENTS.has(analysis.intent) ? cleanText(analysis.product_query) : "") || lastUserQuery;
    ...
    if (analysis.intent === "product_search") {
      console.log("store-chat direct product response", {
        requestId,
        fallback: isProductBrowseFallbackContext(productsContext),
        ms: Date.now() - startedAt,
      });
      return openAiStyleSseFallback(productsContext);
    }
