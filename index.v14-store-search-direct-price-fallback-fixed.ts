...
const getProductFallbackBrowseMessage = (query: string, isArabic: boolean) => {
  ...
  return isArabic
    ? `لأدق نتيجة آمنة، افتح ${target.markdown} وتصفح الخيارات المتاحة هناك. يمكنك أيضًا كتابة اسم الماركة أو الموديل بشكل أدق وسأبحث لك مباشرة في بيانات المول.`
    : `For the safest accurate result, open ${target.markdown} and browse the available options there. You can also send a more specific brand or model name and I’ll search the mall data directly.`;
};

const isProductBrowseFallbackContext = (value: string) => {
  const normalized = normalizeIntentText(value);
  return normalized.includes(normalizeIntentText("لأدق نتيجة آمنة")) || normalized.includes(normalizeIntentText("safest accurate result"));
};
...
8. إذا لم تجد معلومة أو نتيجة مؤكدة، وجّه العميل لرابط مناسب داخل المول بصياغة إيجابية، ولا تستخدم عبارة "لا يوجد منتجات".
