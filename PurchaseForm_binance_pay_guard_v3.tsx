...
const PurchaseForm = ({ onOrderSuccess, onAvailableStarsChange }: PurchaseFormProps) => {
  ...
  const WALLET_NETWORK_PREFERENCE_KEY = 'purchase_wallet_network_choice_v1';
  ...
  const readBep20Preference = useCallback(() => {
    try {
      const stored = sessionStorage.getItem(BEP20_BINANCE_PREFERENCE_KEY);
      if (stored === 'binance_internal' || stored === 'bep20') return stored;
      return null;
    } catch {
      return null;
    }
  ...
  }, [readBep20Preference, resolveWalletNetwork, saveWalletPreference]);
  ...
  const handleWalletsNetworkClick = useCallback(() => {
    ...
    if (preferredWallet === 'bep20' && !readBep20Preference()) {
      walletNetworkBeforeModalRef.current = preferredWallet;
      setRememberBep20Choice(true);
      setShowBep20BinanceModal(true);
    }
  }, [readBep20Preference, resolveWalletNetwork, usdtNetwork]);
  ...
  const handleBep20NetworkDecision = useCallback((useBinance: boolean) => {
    ...
    setShowBep20BinanceModal(false);
  }, [rememberBep20Choice, saveBep20Preference, saveWalletPreference]);
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 px-4 py-6">
          <div className="w-full max-w-md rounded-3xl border border-border bg-background p-5 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                  {lang === 'ar'
                    ? 'إذا كنت سترسل من Binance Pay الداخلي، اختر نعم وسنحوّلك مباشرة إلى التحويل الداخلي خارج السلسلة.'
                    : 'If you are sending from Binance Pay internal transfer, choose Yes and we will switch you to the internal off-chain transfer flow.'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              ...
              >
                {lang === 'ar' ? 'نعم، من Binance Pay' : 'Yes, from Binance Pay'}
              </button>
              <button
                type="button"
