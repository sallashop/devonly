...
async function storeUsedHash(
  ...
  if (error) {
    if (error.code === "23505") {
      throw new Error("تم استخدام هاش المعاملة هذا في طلب سابق (مرفوض أمنياً)");
    }
...
async function markPaidOnly(
  ...
  if (!isApproved) {
    console.error(`❌ [Hash Error] TxHash: ${cleanHash} | Details: Amount mismatch. Expected: ${expectedAmount}, Actual: ${actualHumanAmount}`);
    return apiError(
      "AMOUNT_MISMATCH",
  ...
  await storeUsedHash(supabase, cleanHash, orderId, actualHumanAmount);
  ...
  console.log(`✅ [Success] TxHash: ${cleanHash} | OrderID: ${orderId} | Network: ${network.toUpperCase()} | Amount: ${actualHumanAmount} USDT | Partial: ${isPartial}`);
...
async function verifyOkxInternal(transferId: string, orderId: string, orderData: any, supabase: any) {
  ...
  // 1. البحث في سجل الإيداعات (لتحويلات العناوين الداخلية)
  try {
    ...
    if (depositData?.code === '0' && Array.isArray(depositData?.data)) {
      matchedDeposit = depositData.data.find((dep: any) => 
        dep.depId === cleanId || 
        dep.txId === cleanId || 
        dep.internalId === cleanId || 
        dep.fromWdId === cleanId || // ✅ التقاط الـ fromWdId
        (dep.txId && String(dep.txId).includes(cleanId)) // ✅ التقاط أي جزء من الـ txId
      );
      if (matchedDeposit) console.log(`✅ [OKX Internal] Found by ID in deposit-history`);

      // الفخ السحري
      if (!matchedDeposit) {
        console.log(`⚠️ [OKX Internal] ID not found in deposit-history. Searching by exact amount...`);
        matchedDeposit = depositData.data.find((dep: any) => 
          dep.state === '2' && Math.abs(Number(dep.amt) - expectedAmount) < 0.0001
        );
        if (matchedDeposit) console.log(`✅ [OKX Internal] Found by AMOUNT in deposit-history`);
      }
    }
  } catch(e) { console.error(`❌ [OKX Internal] Deposit API Error:`, e); }

  // 2. البحث في سجل الحساب Bills (لتحويلات الـ UID)
  if (!matchedDeposit) {
    try {
      ...
      if (billsData?.code === '0' && Array.isArray(billsData?.data)) {
        matchedDeposit = billsData.data.find((bill: any) => 
          (bill.billId === cleanId) && Number(bill.balChg) > 0
        );
        if (matchedDeposit) console.log(`✅ [OKX Internal] Found by ID in bills`);

        // الفخ السحري في الفواتير
        if (!matchedDeposit) {
          console.log(`⚠️ [OKX Internal] ID not found in bills. Searching by exact amount...`);
          matchedDeposit = billsData.data.find((bill: any) => 
            Number(bill.balChg) > 0 && Math.abs(Number(bill.balChg) - expectedAmount) < 0.0001
          );
          if (matchedDeposit) console.log(`✅ [OKX Internal] Found by AMOUNT in bills`);
        }
      }
    } catch(e) { console.error(`❌ [OKX Internal] Bills API Error:`, e); }
  }

  if (!matchedDeposit) {
    console.error(`❌ [OKX Internal] Transfer NOT FOUND. ID: ${cleanId}, Expected: ${expectedAmount}`);
    return apiError("OKX_TRANSFER_NOT_FOUND", "لم يتم العثور على التحويل الداخلي. تأكد من الرقم المرجعي أو تواصل مع الدعم.", 404);
  }
  ...
  const amtStr = matchedDeposit.amt || matchedDeposit.balChg;
  const transferUnits = parseDecimalToUnits(amtStr, TRC20_DECIMALS);

  if (transferUnits === null) return apiError("PARSE_ERROR", "خطأ في قراءة المبلغ من المنصة", 500);

  const finalCleanId = matchedDeposit.depId || matchedDeposit.billId || cleanId;
  return await markPaidOnly(supabase, finalCleanId, orderId, transferUnits, TRC20_DECIMALS, "okx_internal", orderData, {
    internalOverpayTolerance: 0.2,
  });
}
...
async function verifyBinanceInternal(transferId: string, orderId: string, orderData: any, supabase: any) {
  ...
  // 1. فحص سجل الإيداعات العادي (Deposit History)
  try {
    ...
    if (Array.isArray(depositData)) {
      matchedDeposit = depositData.find((dep: any) => 
        (dep.txId === cleanId || dep.id === cleanId) && (dep.status === 1 || dep.status === 6)
      );
      if (matchedDeposit) console.log(`✅ [Binance Internal] Found by ID in deposit/hisrec`);

      if (!matchedDeposit) {
        console.log(`⚠️ [Binance Internal] ID not found in deposit/hisrec. Searching by exact amount...`);
        matchedDeposit = depositData.find((dep: any) => 
          (dep.status === 1 || dep.status === 6) && Math.abs(Number(dep.amount) - expectedAmount) < 0.0001
        );
        if (matchedDeposit) console.log(`✅ [Binance Internal] Found by AMOUNT in deposit/hisrec`);
      }
      if (matchedDeposit) amountField = matchedDeposit.amount;
    }
  } catch (e) { console.error(`❌ [Binance Internal] Deposit API Error`, e); }

  // 2. فحص سجل Binance Pay
  if (!matchedDeposit) {
    try {
      ...
      if (payData?.code === "000000" && Array.isArray(payData?.data)) {
        // تم إزالة شرط الـ status لأن بينانس لا ترسله في هذا السجل
        matchedDeposit = payData.data.find((tx: any) => 
          (tx.orderId === cleanId || tx.transactionId === cleanId) && tx.currency === 'USDT'
        );
        if (matchedDeposit) console.log(`✅ [Binance Internal] Found by ID in pay/transactions`);

        if (!matchedDeposit) {
          console.log(`⚠️ [Binance Internal] ID not found in pay/transactions. Searching by exact amount...`);
          matchedDeposit = payData.data.find((tx: any) => 
            tx.currency === 'USDT' && Math.abs(Number(tx.amount) - expectedAmount) < 0.0001
          );
          if (matchedDeposit) console.log(`✅ [Binance Internal] Found by AMOUNT in pay/transactions`);
        }
        if (matchedDeposit) amountField = matchedDeposit.amount;
      }
    } catch (e) { console.error(`❌ [Binance Internal] Pay API Error`, e); }
  }

  if (!matchedDeposit) {
    console.error(`❌ [Binance Internal] Transfer NOT FOUND. ID: ${cleanId}, Expected: ${expectedAmount}`);
    return apiError("BINANCE_TRANSFER_NOT_FOUND", "لم يتم العثور على التحويل الداخلي. تأكد من الرقم المرجعي أو تواصل مع الدعم.", 404);
  }

  const transferUnits = parseDecimalToUnits(amountField, BINANCE_DECIMALS);
  if (transferUnits === null) return apiError("PARSE_ERROR", "خطأ في قراءة المبلغ من المنصة", 500);

  const finalCleanId = matchedDeposit.orderId || matchedDeposit.txId || matchedDeposit.id || cleanId;
  return await markPaidOnly(supabase, finalCleanId, orderId, transferUnits, BINANCE_DECIMALS, "binance_internal", orderData, {
    internalOverpayTolerance: 0.2,
  });
}
