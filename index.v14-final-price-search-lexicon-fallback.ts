...
2. If the user asks about a product category/type/name, set product_search and put the clean searchable product phrase ONLY in product_query. Remove verbs and filler words. Examples: "يسأل عن شاي العروسه" => product_query="شاي العروسه"; "هل فيه منتجات مكتبه" => product_query="منتجات مكتبه"; "ايه الشاي المتوفر" => product_query="شاي"; "ارخص موابيل ايه" => product_query="موبايل"; "ارخص موبايبل ايه" => product_query="موبايل"; "موبايلات سامسون" => product_query="موبايل سامسونج"; "فيه تليفونات؟" => product_query="تليفون"; "شاي العروسة بكام" => product_query="شاي العروسة".
...
const PRODUCT_COMMERCIAL_INTENT_WORDS = [
  ...
  "اشتري", "شراء", "سعر", "سعره", "سعرها", "بكام", "بكم", "كام", "كم", "ثمن", "تكلفة", "تكلفه", "ارخص", "الأرخص", "افضل", "أفضل", "رشح", "اقترح",
  "want", "need", "buy", "find", "search", "show", "price", "how much", "cost", "cheapest", "best", "recommend",
];

const PRODUCT_PRICE_QUERY_WORDS = [
  "بكام", "بكم", "كام", "كم", "سعر", "سعره", "سعرها", "الثمن", "ثمن", "تكلفة", "تكلفه",
  "price", "how much", "cost", "how much is", "how much are",
];
...
const getProductSearchDecision = (query: string): ProductSearchDecision => {
  ...
  const hasPriceQuestion = hasAnyNormalizedWord(normalized, PRODUCT_PRICE_QUERY_WORDS);
  const hasNonPriceToken = normalized
    .split(" ")
    .some((word) => word.length >= 2 && !PRODUCT_PRICE_QUERY_WORDS.some((priceWord) => normalizeIntentText(priceWord) === word));
  ...
  if (hasPriceQuestion) confidence += 0.35;

  // أسئلة السعر مثل "شاي العروسة بكام" يجب أن تدخل مسار البحث حتى لو لم تكن الماركة موجودة في المعجم.
  if (hasPriceQuestion && hasNonPriceToken && !hasHardNonProduct && !hasSoftNonProduct) {
