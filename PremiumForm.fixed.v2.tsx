...
const PremiumForm = ({ onOrderSuccess }: PremiumFormProps) => {
  ...
  const totalCostDecimals = isWalletMethod && currency === 'egp' ? 0 : decimals;
                ...
                >
                  <div className="mb-5">
                    ...
                    {telegramName && !verifyingUser && hasPremium && (
                      <div className="flex items-start gap-2 mt-3 px-3 py-2.5 bg-amber-500/10 rounded-xl text-muted-foreground font-bold text-xs border-none shadow-inner">
                      ...
                      <div className="bg-muted/40 rounded-[1.5rem] p-5 border-none shadow-inner">
                        ...
                        >
                          ...
                          <span className="whitespace-normal text-center leading-tight">
                            {lang === 'ar' 
                              ? `اضغط لفتح انستا باي ودفع مبلغ ${totalCost.toFixed(totalCostDecimals)} ${currencyLabel}` 
                              : `Tap to open InstaPay & pay ${totalCost.toFixed(totalCostDecimals)} ${currencyLabel}`}
                          </span>
                        </a>
                      </div>
                  ...
                  <div className="mb-5 space-y-4 overflow-hidden border-none">
                    <div className={`bg-gradient-to-br rounded-[1.5rem] p-5 sm:p-6 shadow-inner border-none ${paymentMethod === 'solana' ? 'from-[#14F195]/10 to-[#9945FF]/10' : 'from-[#0098EA]/10 to-[#0098EA]/5'}`}>
                      ...
                      <div className="text-sm text-foreground/80 font-bold leading-relaxed space-y-3 mb-5 border-none">
                        <p>
                          {lang === 'ar'
                            ? `اضغط لتأكيد دفع مبلغ قدره ${totalCost.toFixed(totalCostDecimals)} USDT. سيتم تجهيز المعاملة الخاصة بك في ثواني.`
                            : `Click to confirm payment of ${totalCost.toFixed(totalCostDecimals)} USDT. Your transaction will be prepared in seconds.`}
                        </p>
                          ...
                          <div className="flex items-start gap-2 border-none">
                            ...
                            <div className="text-[11px] leading-relaxed text-muted-foreground border-none space-y-2">
                              ...
                              {usdtNetwork === 'okx_internal' || usdtNetwork === 'binance_internal' ? (
                                ...
                                <ul className="list-disc pl-4 space-y-1">
                                  <li>{lang === 'ar' ? 'أرسل المبلغ كما هو ظاهر.' : 'Send the amount exactly as shown.'}</li>
                      ...
                      <div className="mt-4 bg-secondary rounded-[1.25rem] p-4 shadow-inner border-none overflow-hidden">
                        <div className="flex justify-between items-center gap-2 border-none">
                          ...
                          <div className="flex items-center gap-2 min-w-0 flex-1 justify-end overflow-hidden border-none">
                            ...
                            >
                              {totalCost.toFixed(totalCostDecimals)} {currencyLabel}
                            </span>
                            <button
                              type="button"
                              onClick={() => handleCopy(totalCost.toFixed(totalCostDecimals), 'amount')}
