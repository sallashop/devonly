...
type ProductMatch = {
  ...
  source?: "merchant" | "admin" | string | null;
...
const formatProductPrice = (product: ProductMatch, piRate?: number) => {
  ...
  // 1. الأولوية الأولى: الحساب الديناميكي المباشر (كما تفعل الواجهة تماماً)
  if (localPriceEgp > 0 && piPerEgp > 0) {
    return Number((localPriceEgp * piPerEgp).toFixed(5)).toString();
  }

  // 2. الأولوية الثانية: السعر المخزن مسبقاً إذا لم يتوفر السعر المحلي
...
const getProductLink = (product: ProductMatch, name: string) => {
  const source = product.source === "merchant" ? "merchant" : product.source === "service" ? "service" : "admin";

  if (source === "merchant") return `[${name}](/merchant-product/${product.id})`;
  if (source === "service") return `[${name}](/service-product/${product.id})`;
  return `[${name}](/product/${product.id})`;
};

const getProductCategoryLink = (product: ProductMatch) => {
  const categoryId = cleanText(product.category_id || product.category);
  const source = product.source === "merchant" ? "merchant" : product.source === "service" ? "service" : "admin";

  if (source === "merchant") {
    return categoryId ? `/merchant-products?category=${categoryId}` : "/merchant-products";
  }

  if (source === "service") {
    return categoryId ? `/products?type=service&category=${categoryId}` : "/products?type=service";
  }
  ...
  return cleanProducts
    .map((product, index) => {
      ...
      return [
        ...
        categoryLink ? `   - ${isArabic ? "للمزيد من نفس القسم" : "More from this category"}: [${category}](${categoryLink})` : "",
        `   - ${isArabic ? "السعر" : "Price"}: ${price} Pi`,
...
const fetchProductsByText = async (
  ...
  const uniqueProducts = uniqueBy(((data || []) as ProductSearchRpcRow[]), (product) => `${product.source || "admin"}:${cleanText(product.id)}`)
    .filter((product) => product.in_stock === true && toNumber(product.stock_quantity) > 0);
...
const fetchServiceProductsByText = async (
  ...
  for (const safeQuery of searchTerms.slice(0, 8)) {
    const filters = [
    ...
    const selectFields = "id, name, name_ar, description, description_ar, image, category_id, min_amount_egp, max_amount_egp, is_featured";
    ...
    if (result.error) {
      console.warn("service_products text search skipped:", result.error.message);
      continue;
    }

    const serviceProducts = ((result.data || []) as any[]).map((service) => ({
      id: service.id,
      name: service.name,
      ...
      price: null,
      local_price_egp: service.min_amount_egp,
      original_price: null,
      source: "service",
      in_stock: true,
      stock_quantity: 999,
      shipping_type: "free",
      tags: ["service", "خدمة", "خدمات"],
      is_featured: service.is_featured,
    } as ProductMatch));

    collectedServices.push(...serviceProducts);
  }

  const searchTermsForScore = extractProductSearchTerms(lastUserQuery);

  return uniqueBy(collectedServices, (product) => `service:${cleanText(product.id)}`)
    .map((product) => ({
      product,
      score: scoreProductForQuery(product, searchTermsForScore, lastUserQuery),
    }))
    .filter((item) => Number.isFinite(item.score) && item.score >= Math.max(25, MIN_PRODUCT_CONTEXT_SCORE - 20))
    .sort((a, b) => {
      const scoreDiff = b.score - a.score;
      if (scoreDiff !== 0) return scoreDiff;
      return toNumber(a.product.local_price_egp || a.product.price) - toNumber(b.product.local_price_egp || b.product.price);
...
const enrichProductsWithCategoryNames = async (
  ...
  if (error || !data?.length) {
    if (error) console.warn("category enrichment skipped:", error.message);
    return products;
  }
...
const fetchProductsContext = async (
  ...
  // 1. تشغيل البحث النصي والخدمي (Text & Service) أولاً لضمان جلب البيانات المحدثة كاملة (local_price_egp)
  ...
  // 2. تشغيل البحث المتجهي (Vector) ثانياً
  if (queryEmbedding) {
  ...
  // إزالة التكرار مع الحفاظ على أولوية نتائج البحث النصي المحدثة
  let uniqueProductsRaw = uniqueBy(products, (product) => `${product.source || "admin"}:${cleanText(product.id)}`);

  // 3. طبقة حماية إضافية: جلب السعر المحلي لأي منتج قادم من البحث المتجهي ولم يتضمن السعر المحلي بالخطأ
  const missingAdminIds = uniqueProductsRaw.filter(p => p.source !== "merchant" && p.source !== "service" && p.local_price_egp == null).map(p => p.id);
  const missingMerchantIds = uniqueProductsRaw.filter(p => p.source === "merchant" && p.local_price_egp == null).map(p => p.id);

  if (missingAdminIds.length > 0 || missingMerchantIds.length > 0) {
    const [adminRes, merchantRes] = await Promise.all([
      missingAdminIds.length > 0 ? supabase.from("products").select("id, local_price_egp").in("id", missingAdminIds) : Promise.resolve({ data: null }),
      missingMerchantIds.length > 0 ? supabase.from("merchant_products").select("id, local_price_egp").in("id", missingMerchantIds) : Promise.resolve({ data: null })
    ]);

    uniqueProductsRaw = uniqueProductsRaw.map(p => {
      if (p.source !== "merchant" && p.source !== "service" && p.local_price_egp == null && adminRes.data) {
        const match = adminRes.data.find((d: any) => d.id === p.id);
        if (match) return { ...p, local_price_egp: match.local_price_egp };
      }
      if (p.source === "merchant" && p.local_price_egp == null && merchantRes.data) {
        const match = merchantRes.data.find((d: any) => d.id === p.id);
        if (match) return { ...p, local_price_egp: match.local_price_egp };
      }
      return p;
    });
  ...
  const availableProducts = enrichedProducts.filter((product) => product.in_stock === true && toNumber(product.stock_quantity) > 0);
...
4. إذا سأل عن منتج، اقترح فقط المنتجات الموجودة في قسم "المنتجات المطابقة لبحث العميل".
...
7. حافظ على روابط Markdown كما هي.
...
4. For product questions, suggest only products listed under "Matching Products".
...
7. Keep Markdown links exactly as provided.
