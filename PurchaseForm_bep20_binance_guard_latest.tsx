...
const PurchaseForm = ({ onOrderSuccess, onAvailableStarsChange }: PurchaseFormProps) => {
  ...
  const totalCost = useMemo(() => (
    paymentMethod === 'instapay' && currency === 'egp'
      ...
      : (paymentMethod === 'usdt' && !isInternalUsdtTransfer)
        ...
        : baseTotalCost
  ), [baseTotalCost, cryptoFraction, currency, isInternalUsdtTransfer, paymentMethod]);
  ...
  const BEP20_BINANCE_PREFERENCE_KEY = 'purchase_bep20_binance_choice_v1';
  const [showBep20BinanceModal, setShowBep20BinanceModal] = useState(false);
  const [rememberBep20Choice, setRememberBep20Choice] = useState(true);

  const readBep20Preference = useCallback(() => {
    try {
      return sessionStorage.getItem(BEP20_BINANCE_PREFERENCE_KEY);
    } catch {
      return null;
    }
  }, []);

  const saveBep20Preference = useCallback((value: 'bep20' | 'binance_internal') => {
    try {
      sessionStorage.setItem(BEP20_BINANCE_PREFERENCE_KEY, value);
    } catch {
      // ignore
    }
  }, []);

  const clearBep20Preference = useCallback(() => {
    try {
      sessionStorage.removeItem(BEP20_BINANCE_PREFERENCE_KEY);
    } catch {
      // ignore
    }
  }, []);
  ...
  const openBep20NetworkGuard = useCallback(() => {
    const stored = readBep20Preference();
    if (stored === 'binance_internal' || stored === 'bep20') {
      setUsdtNetwork(stored);
      return;
    }

    if (enabledNetworks.bep20) {
      setUsdtNetwork('bep20');
    } else if (enabledNetworks.trc20) {
      setUsdtNetwork('trc20');
    } else if (enabledNetworks.aptos) {
      setUsdtNetwork('aptos');
    }

    setRememberBep20Choice(true);
    setShowBep20BinanceModal(true);
  }, [enabledNetworks.aptos, enabledNetworks.bep20, enabledNetworks.trc20, readBep20Preference]);
  ...
  const handleBep20NetworkDecision = useCallback((useBinance: boolean) => {
    const nextNetwork: 'bep20' | 'binance_internal' = useBinance ? 'binance_internal' : 'bep20';

    if (rememberBep20Choice) {
      saveBep20Preference(nextNetwork);
    }

    setUsdtNetwork(nextNetwork);
    setShowBep20BinanceModal(false);
  }, [rememberBep20Choice, saveBep20Preference]);
  ...
  const formatPaymentAmount = useCallback((value: number) => {
    ...
    return normalizedValue.toFixed(paymentMethod === 'instapay' && currency === 'egp' ? 0 : decimals);
  }, [currency, decimals, paymentMethod]);
                      ...
                      <div className="text-xs text-destructive font-bold leading-relaxed border-none">
                          ...
                          {lang === 'ar' ? (
                            <>
                              أرسل فقط USDT عبر <span className="text-primary font-black border-none">
                                {usdtNetwork === 'bep20' ? 'BEP20 (BSC)' : usdtNetwork === 'trc20' ? 'TRC20 (Tron)' : usdtNetwork === 'aptos' ? 'Aptos' : usdtNetwork === 'okx_internal' ? 'التحويل الداخلي (OKX)' : 'التحويل الداخلي (Binance)'}
                            ...
                            <>
                              Only send USDT on the <span className="text-primary font-black border-none">
                                {usdtNetwork === 'bep20' ? 'BEP20 (BSC)' : usdtNetwork === 'trc20' ? 'TRC20 (Tron)' : usdtNetwork === 'aptos' ? 'Aptos' : usdtNetwork === 'okx_internal' ? 'OKX Internal' : 'Binance Internal'}
                        ...
                        {usdtNetwork === 'bep20' && (
                          <p className="mt-2 text-[11px] leading-relaxed border-none">
                            {lang === 'ar' ? (
                              <>
                                إذا كنت سترسل من بينانس على العنوان، استخدم{' '}
                                <button
                                  type="button"
                                  onClick={openBep20NetworkGuard}
                                  className="font-black text-primary underline underline-offset-2"
                                >
                                  طريقة الدفع بينانس الداخلي من هنا
                                </button>
                                .
                              </>
                            ) : (
                              <>
                                If you are sending from Binance to this address, use{' '}
                                <button
                                  type="button"
                                  onClick={openBep20NetworkGuard}
                                  className="font-black text-primary underline underline-offset-2"
                                >
                                  Binance internal transfer here
                                </button>
                                .
                              </>
                            )}
          ...
          <div className="w-full max-w-md rounded-3xl border border-border bg-background p-5 shadow-2xl">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-base font-black text-foreground">
                  {lang === 'ar' ? 'هل سترسل من Binance؟' : 'Will you send from Binance?'}
                </h3>
                <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                  {lang === 'ar'
                    ? 'إذا كنت سترسل من بينانس الداخلية، اختر نعم وسنحوّلك مباشرة إلى التحويل الداخلي خارج السلسلة.'
                    : 'If you are sending from Binance internal transfer, choose Yes and we will switch you to the internal off-chain transfer flow.'}
                </p>
              </div>
            </div>

            <label className="mt-4 flex items-center gap-2 rounded-2xl bg-muted/40 px-3 py-2 text-sm font-medium text-foreground">
              <input
                type="checkbox"
                checked={rememberBep20Choice}
                onChange={(e) => setRememberBep20Choice(e.target.checked)}
                className="h-4 w-4 rounded border-border"
              />
              <span>{lang === 'ar' ? 'تذكر اختياري لهذا الشراء' : 'Remember my choice for this purchase'}</span>
            </label>
            ...
            <div className="mt-5 grid grid-cols-2 gap-3">
              <button
                ...
                className="rounded-2xl bg-primary px-4 py-3 text-sm font-black text-primary-foreground transition hover:opacity-90"
              >
                {lang === 'ar' ? 'نعم، من Binance' : 'Yes, from Binance'}
              </button>
              <button
                type="button"
                onClick={() => handleBep20NetworkDecision(false)}
                className="rounded-2xl bg-secondary px-4 py-3 text-sm font-black text-foreground transition hover:bg-secondary/80"
              >
