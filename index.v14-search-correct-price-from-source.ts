...
const formatProductPrice = (product: ProductMatch, piRate?: number) => {
  ...
  /**
   * تم اقتباس منطق السعر فقط من النسخة التي أرسلتها:
   * piRate يأتي من الواجهة من calculatePrice(1, 'free').priceInPi
   * أي أنه يمثل كم Pi يقابل 1 EGP.
   * لذلك سعر منتجات المدير والتجار في الشات يجب أن يُحسب من:
   * local_price_egp * piRate
   *
   * مهم: لا نستخدم product.price الخام إذا كان local_price_egp موجودًا،
   * لأن بعض السجلات قد يكون فيها price رقمًا محليًا مثل 7500 وليس سعر Pi.
   */
  if (localPriceEgp > 0 && piPerEgp > 0) {
    return (localPriceEgp * piPerEgp).toFixed(2);
  }

  /**
   * fallback آمن فقط عند عدم وجود local_price_egp من الأساس.
   * هذا يحافظ على المنتجات القديمة التي كان price فيها مخزنًا فعلاً كـ Pi،
   * ولا يضرب المنتجات الحديثة التي تعتمد على local_price_egp.
   */
  if (localPriceEgp <= 0 && storedPiPrice > 0) {
    return storedPiPrice.toFixed(2);
  }
  ...
  return cleanProducts
    .map((product, index) => {
      ...
      return [
        ...
        `   - ${isArabic ? "السعر" : "Price"}: ${price === "غير محدد" ? price : `${price} Pi`}`,
...
const fetchProductsContext = async (
  ...
  const textProducts = await fetchProductsByText(supabase, lastUserQuery);
  products.push(...textProducts);

  const searchTerms = extractProductSearchTerms(lastUserQuery);
  const enrichedProducts = await enrichProductsWithCategoryNames(
    supabase,
    uniqueBy(products, (product) => `${product.source || "admin"}:${cleanText(product.id)}`),
  );
...
6-أ. استخدم السعر الموجود في سطر "السعر" داخل سياق المنتج كما هو حرفيًا، ولا تعيد حساب السعر من أي رقم آخر في الاسم أو الوصف أو قاعدة البيانات.
